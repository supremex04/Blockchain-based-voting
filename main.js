...
let contractAddress = "0x8C59a1b5EB6F8aE76540F722718b1E2Be719520c";
let contractAbi = [
  {
    "inputs": [
      {
...
const voteStatus = async() => {
    if(WALLET_CONNECTED != 0) {
        ...
        const signer = provider.getSigner();
        const contractInstance = new ethers.Contract(contractAddress, contractAbi, signer);
        const currentStatus = await contractInstance.getVotingTimeStatus();
...
const getAllCandidates = async() => {
    if(WALLET_CONNECTED != 0) {
        ...
        var candidates = await contractInstance.getCandidatesInfo();
